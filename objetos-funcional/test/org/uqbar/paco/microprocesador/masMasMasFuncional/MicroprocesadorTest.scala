...
import org.uqbar.paco.microprocesador.masMasMasFuncional._
...
class MicroprocesadorTest {
  ...
  var resultado: List[ResultadoDeEjecucion] = _
  ...
  def ejecutarEnMicro(programa: Instruccion*) = resultado = List(Ejecucion1, Ejecucion2).map{_.ejecutar(micro,programa :_*)}

  def assertDatos(expected:Short, position: Int) = for(r <- resultado) r match {
    case Halt(micro) => assertEquals(expected, micro.memoriaDeDatos(position))
    case _ => fail
  }

  def assertRegistros(expectedA:Short, expectedB:Short) = {
    for(r <- resultado) r match {
    case Halt(micro) =>
  ...
  def assertProgramCounter(expectedPc: Int) = for(r <- resultado) r match {
